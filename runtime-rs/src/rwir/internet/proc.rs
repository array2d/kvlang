...
//! /internet/{host}/proc/{pid}/{stream}）；未绑定→继承父终端（保持无捕获时的直通输出）。
...
/// 捕获字节落物理文件 + 该写槽写 @[]uint8 句柄（body=逻辑 /internet/{host}/proc/{pid}/{stream}）。
...
/// @ 句柄兑现（read_at 调）：逻辑 /internet/{host}/proc/{pid}/{stream} → 物理文件读回字节。
