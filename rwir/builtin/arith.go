...
func (o arith) Call(f *rwir.Frame) error {
	...
	if o.concat && len(inputs) == 2 && kvspace.IsCharKind(inputs[0].Kind()) && kvspace.IsCharKind(inputs[1].Kind()) {
		return writeResult(f, kvspace.NewChar(kvspace.KindChar, inputs[0].ValueString()+inputs[1].ValueString()))
	}
