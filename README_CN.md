...
**顶层：`import`、`lib name { }`、`init { }`、`def`、单条指令。** 裸 `if` / `while` / `for` 在顶层自动封装为隐式 `def __init__() { … }`。惯例是定义 `main` 再调用它：
...
**`def func(ra,rb) -> (wa,wb) { … }` = 自定义复合 rwir**，单条 rwir 如 `A + B -> C` 是原子 rwir（一个操作码 + 读参 + 写参）；`def` 把多条 rwir 打包成命名单元，对外暴露相同的箭头接口——`(ra,rb)` 是读参声明，`-> (wa,wb)` 是写参声明。调用 `add(3,4) -> s` 即把实参绑入读槽、写槽映射回调用方帧。没有返回值，只有写参映射：
