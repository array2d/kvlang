...
#### 3.1.1 `strVType.Exec` 中的 `IsNativeOp` 检查永远为 false
...
#### 3.2.3 `copyFunc` O(N×M) KV 操作——每次函数调用的主要开销
...
| # | 改动 | 文件 |
|---|------|------|
| 1 | 删 `strVType.Exec` 中死分支（IsNativeOp 检查） | vtype/str.go |
