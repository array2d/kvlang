...
int kvlangKvGetMember(kvlangKv_t *k, const char *dir, const char *name, kvlangXvalue_t *out) {
    ...
    uint8_t *d;
    uint32_t len;
    if (ref_ok(k) && hot_get(k, dir, name, &d, &len)) {
        out->data = d;
        out->len = len;
        out->borrowed = 1;
        return 0;
    ...
    if (ref_ok(k)) {
        int hit = 0;
        kvlangRefEnt_t *pe = NULL;
        if (nl >= MEMBER_SEP_LEN && memchr(name, 0xC2, nl)) {
            pe = pref_cover(k, key);
            hit = pe && parent_hit_ent(k, pe, key, &d, &len);
        } else if (k->fpar.key) {
            pe = &k->fpar;
            hit = parent_hit_ent(k, pe, key, &d, &len);
        }
        if (hit) {
            if (pe)
                hot_put(k, name, key, pe->block_id, pe->gen);
            out->data = d;
            out->len = len;
            out->borrowed = 1;
            free(heap);
