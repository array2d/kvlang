...
int kvlangKvGetMember(kvlangKv_t *k, const char *dir, const char *name, kvlangXvalue_t *out) {
    ...
    key[dl + nl] = 0;
    /* Multi-char siblings (bsearch lo/hi): ART parent before the 64-slot scan.
     * 1-char non-hot (sieve d) stays leaf-first; parent is the miss path. */
