...
bool kvlangDispatchIsDelegatedOp(kvlangKv_t *kv, const char *opcode) {
    ...
    bool ok = duty.n > 0;
    if (!ok) neg_put(kv, op);
    sl_free(&reg); sl_free(&duty); free(op);
    return ok;
}
...
int kvlangDispatchDelegate(kvlangKv_t *kv, const char *vtid, const char *pc, kvlangRwirInst_t *inst) {
    ...
    if (!kvlangXvalueNone(&decl) && kvlangXvalueKindIs(&decl, KVSPACE_KIND_RWIR)) {
        int dnr = 0, dnw = 0;
    ...
    } else if (!kvlangXvalueNone(&sig) && kvlangXvalueKindIs(&sig, KVSPACE_KIND_RWFUNC)) {
        strlist_t reg = {0}, duty = {0};
        backends_for(kv, op, &reg, &duty);
        sl_free(&reg);
        if (duty.n == 0) {
        ...
        free(op);
        return rc;
    } else if (!kvlangXvalueNone(&decl) && !kvlangXvalueKindIs(&decl, KVSPACE_KIND_RWIR)) {
