...
// strCharOp: string.char(s, i) -> str —— 返回 s 的第 i 个 Unicode 字符（单字符字符串）。
...
// strOrdOp: string.ord(c) -> int —— 返回单字符字符串的 Unicode 码点值（fix-024 配套；Python 阵营，见 p7）。
...
// strCmpOp: string.cmp(a, b) -> int —— C 语义：a<b 返 -1，相等返 0，a>b 返 1（按字节序）。
...
// strStrOp: string.find(hay, needle) -> int —— C 名 + 索引语义（C 返指针无法值语义化，
...
// strLenOp: string.len(s) -> int —— 返回字符串的 Unicode 字符数（RC4：由字节数改为字符数）。
...
// strSliceOp: string.slice(s, i, j) -> string (substring s[i:j] by Unicode characters)
...
// strConcatOp: string.concat(a, b) -> string
