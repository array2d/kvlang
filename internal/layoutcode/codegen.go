...
func HandleCall(ctx context.Context, kv kvspace.KVSpace, vtid, pc string, inst *op.Instruction) string {
	...
	for i, param := range formalParams.Reads {
		if i+1 < len(inst.Reads) { bindings[param] = inst.Reads[i+1] }
	}
	for i, param := range formalParams.Writes {
		if i < len(inst.Writes) { bindings[param] = inst.Writes[i] }
	}

	compiled := mgetAll(ctx, kv, keytree.OpBackendFunc(backend, funcName))
	if len(compiled) == 0 { compiled = mgetAll(ctx, kv, keytree.SrcFunc(funcName)) }
...
func replaceParams(params []string, bindings map[string]string) {
	for i, p := range params {
		if v, ok := bindings[p]; ok { params[i] = v }
	}
}
...
func mgetAll(ctx context.Context, kv kvspace.KVSpace, base string) []string {
	...
	type ik struct{ key string; index int }
	var sorted []ik
	for _, k := range keys {
	...
	for i, s := range sorted { ordered[i] = s.key }
	if len(ordered) == 0 { return nil }
	vals, _ := kv.MGet(ordered...)
	result := make([]string, 0, len(vals))
	for _, v := range vals {
		if s, ok := v.(string); ok { result = append(result, s) }
	}
	return result
}
